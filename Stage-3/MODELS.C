...
/* Author: Depanshu
 *
 * Purpose: Sets the player's jump property to true and changes the vertical
 *          velocity of the player with the input velocity
 */
...
*          It reutrn false if the player is falling or player had requested to jump and its jump property was turned on
*
*/
bool is_player_not_falling(Model *model, short player_num) {
    int i = 0;

    if(model->player[player_num].is_jumping == TRUE) {
            return FALSE;
    }
