...
void handle_no_special_key(unsigned char* value, KEY_STATE *key_state);
void handle_caps_lock_on(unsigned char* value, KEY_STATE *key_state);
void handle_shift_held(unsigned char* value, KEY_STATE *key_state);
...
unsigned char get_mouse_button_value(){
    unsigned char value = mouse_button;
    mouse_button = 0;
    return value;
}
