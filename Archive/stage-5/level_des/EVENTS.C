...
void move_a_single_active_triangle(Model* model, short triangle_num) {
	...
	for(i; i< model->triangles[triangle_num].hor_vel; i++){
		...
		if (model->triangles[triangle_num].hot_spot.x_pos == 0) {
			remove_triangle(model, triangle_num);
			return;
		}
...
/*********************************************************
 * Purpose: Removes a single instance of a triangle from an array
 *
 * Input: Pointer to a Model struct
 * Output: Updated array of triangles[] from Model
 *
 * Details: Shifts all triangles so that the front of the array is always at index 0
 * 
 *******************************************************/
void remove_triangle (Model* model, short triangle_num) {
	...
	if (model->triangles[triangle_num].hot_spot.x_pos == 0) {
		for (i; i < model->num_active_triangles; i++) {
			if (model->num_active_triangles > 0) {
				model->triangles[triangle_num + i] = model->triangles[(triangle_num + 1) + i];
			}
		}
		clear_triangle(&(model->triangles[triangle_num + i]));
		model->num_active_triangles--;
	}
}
/*********************************************************
 * Purpose: Clears a single instance of a triangle
 *
 * Input: Pointer to a Triangle struct
 * Output: Updated positions of all vertices.
 *
 * Details: A Triangle that has its values set to all zero should be considered
 * 			uninitialized and should never be used until initialized.
 *******************************************************/

void clear_triangle (Triangle* triangle) {
	triangle->hot_spot.x_pos = 0;
	triangle->hot_spot.y_pos = 0;

	triangle->bot_left.x_pos = 0;
	triangle->bot_left.y_pos = 0;

	triangle->bot_right.x_pos = 0;
	triangle->bot_right.y_pos = 0;

	triangle->top.x_pos = 0;
	triangle->top.y_pos = 0;
...
void move_a_single_platform(Model* model,short platform_number) {
	...
	for(i; i< model->platform[platform_number].hor_vel; i++){
		...
		if (model->platform[platform_number].top_left.x_pos == 0) {
			remove_platform(model, platform_number);
			return;
		}
		for (k = 0; k < model->num_active_players; k++) {
 ...
 *
 * Input: Pointer to a Model struct
 * Output: Updated array of platform[] from Model
 *
 * Details: Shifts all triangles so that the front of the array is always at index 0
 * 
 *******************************************************/
void remove_platform (Model* model, short platform_number) {
	...
	if (model->platform[platform_number].top_left.x_pos == 0) {
		for (i; i < model->num_active_platforms; i++) {
			if (model->num_active_platforms > 0) {
				model->platform[platform_number + i] = model->platform[(platform_number + 1) + i];
			}
		}
		clear_platform(&(model->platform[platform_number + i]));
		model->num_active_platforms--;
	}
}
/*********************************************************
 * Purpose: Clears a single instance of a platform
 *
...
void clear_platform (Platform* platform) {
	platform->top_left.x_pos = 0;
	platform->top_left.y_pos = 0;

	platform->top_right.x_pos = 0;
	platform->top_right.y_pos = 0;

	platform->bot_left.x_pos = 0;
	platform->bot_left.y_pos = 0;

	platform->bot_right.x_pos = 0;
	platform->bot_right.y_pos = 0;
 ...
 *
 * Input: Pointer to the game model structure.
 * Output: Updates each active player's position if they are jumping or falling.
 *********************************************************/
...
void move_one_player(Model *model, short player_num) {
	...
	if (model->player[player_num].vert_vel == -15) {
		return; 
	} else {
