...
#define GROUND_WIDTH      640
#define GROUND_HEIGHT    128
...
/* Calculates Y on the hypotenuse from the bottom left vertex based on POINT_X*/
#define Y_ON_HYPOTENUSE_FROM_BOT_LEFT(POINT_X, TRIANGLE) \
(-(((POINT_X) - (TRIANGLE)->bottom_left.x_pos) << 2) + (TRIANGLE)->bottom_left.y_pos)

/* Calculates Y on the hypotenuse from the top right vertex based on POINT_X*/
#define Y_ON_HYPOTENUSE_FROM_TOP(POINT_X, TRIANGLE) \
( ( ((POINT_X) - (TRIANGLE)->top_right.x_pos) << 2) + (TRIANGLE)->top_right.y_pos)
