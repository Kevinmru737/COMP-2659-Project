 ...
 *          (triangles and platforms) in the game model.
 * 
 * Details: A collision with platforms is detected through vertical overlap,
 *          while for triangles, checking any player's corner inside the
 *          triangle indicates a collision.It is assumed that the player is
 *          the first object in the model
 *
 * Input: 
 *        - model: Pointer to the Model struct, containing game state and objects.
 *
 * Output: 
 *        - Returns TRUE if a fatal collision is detected with any object marked
 *         as near the player, else FALSE.
 *
 * Assumptions:
...
/* Skip if the object is not near the player or if there's no vertical overlap*/
		...
		if(object->type == TRIANGLE) {
            /*check if each vertex of the player is inside the triangle*/
        ...
        /* Check for vertical overlap with the platform, excluding the very top edge.*/
