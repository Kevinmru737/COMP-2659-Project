...
void update_triangle_pos(Triangle* triangle) {
	...
	/*wraps the triangle back to right border of the screen if it has reached the left edge of the screen*/
...
void update_platform_pos(Platform* platform) {
	...
	/*wraps the platform back to right border of the screen if it has reached the left edge of the screen*/
...
/* Author: Depanshu
*  Purpose: THe platform will hit the left edge of the screen, this function will sets its coordinates back to right edge
*/
...
*
* Details:The collision is detected by checking if any of the player's corners are inside the triangle.
*	      It uses a helper function which accepts a vertex and checks if it is inside the triangle. The function
*		  calls this function for all 4 vertex of player. It also checks for a collision which can occur if
*		   when the player bottom edge collides with the triangle's top vertex
*
*/
...
*
* Restriction: Assumes that the point is in vicinity of the triangle. 
