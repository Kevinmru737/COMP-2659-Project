 ...
 *          their states based on current conditions and performs collision
 *          detection between the player and obstacles (platforms and triangles).
 *
 * Input: A pointer to a Model struct containing the game's current state,
 *        including all game objects. A pointer to a boolean (`fatal_collision`)
 *        to flag if a fatal collision occurs during updates.
 * Output: Updates each game object based on its type (player, platform, triangle)
 *         and current state. Sets `fatal_collision` to TRUE if a collision between
 *         the player and an obstacle is detected.
 *
 * Details: This function loops through each active game object, applying specific
 ...
 *          state transitions and velocity changes. Collision detection is performed
 *          by calling `platform_collision_unsafe` and `player_triangle_collision`
 *          functions to check for collisions. If a collision is detected, the
 *          `fatal_collision` flag is set to TRUE.
        ...
        if(model->game_objects[i].type == PLAYER) {
            switch(model->game_objects[i].state) {
                ...
                case (PLAYER_STATE_MOVING_DOWN):
                    update_player_state_moving_down(model, i);
                    break;
                case (PLAYER_STATE_ON_PLATFORM):
                    update_player_state_on_platform(model, i);
                    break;
                default:
                    break;
            }

            if( platform_collision_unsafe(model, i) == TRUE|| player_triangle_collision( model, i) == TRUE) {
...
/*
    if(platform_collision_unsafe(model, 0) == TRUE || player_triangle_collision( model, 0) == TRUE ) {
        *fatal_collision = TRUE;
    }

 for testing commented this out, should turn back on after*/
 ...
 *        including all game objects, and a short (`player_num`) indicating the
 *        player's index within the game model's object array.
 * Output: Modifies the player's vertical position based on their current velocity.
 ...
 *          on velocity and apply gravity until a collision occurs.
 *********************************************************/
void update_player_state_moving_down(Model * model, short player_num) {
    ...
    if(check_player_safe_collision_with_platform(model, player_num) == FALSE) {
        if (new_y_pos_bottom >= GROUND_POS) {
            ...
            if (player->object_velocity > -30) {
                player->object_velocity -= 5;
            }
        }
...
/*********************************************************
 * Purpose: Checks for collisions between the player and platforms to determine
 *          if the player lands on a platform during their descent.
 *
 * Input: A pointer to the Model struct (`model`) representing the game's current
 *        state, and a short (`player_num`) indicating the player's index within
 *        the game model's object array.
 * Output: Returns TRUE if the player collides with the top of a platform, otherwise
 *         FALSE. Upon collision, updates the player's position to stand on the
 *         platform and adjusts their velocity and state to reflect this new position.
 *
...
bool check_player_safe_collision_with_platform(Model * model, short player_num) {
    ...
    short i = 1;
    for (i; i < model->num_active_objects; i++) {
        if(model->game_objects[i].type == TRIANGLE || model->game_objects[i].state != OBJECT_NEAR_PLAYER) continue;

            platform_y_pos_top = model->game_objects[i].top_right.y_pos;

            if (player->bottom_left.y_pos <= platform_y_pos_top && new_y_pos_bottom >= platform_y_pos_top) {
...
/*********************************************************
 * Purpose: Triggers a jump for a player if they are safely on the ground or a platform.
 *
 * Input: Pointer to a Model struct representing the game model, and a short indicating
 *        the player's index within the game model's object array.
 * Output: Updates the vertical velocity of the player to an initial jump velocity if the player
 *         is safely on a surface or ground. Does nothing if the player is not in a safe position to jump.
 *
 * Details: Checks if the player's current state is either on the ground or on a platform. If true,
 *          the player's vertical velocity is set to an initial jump velocity, and their state is
...
void jump_request(Model * model, short player_num) {
    ...
    if (model->game_objects[player_num].state == PLAYER_STATE_ON_PLATFORM ||
        model->game_objects[player_num].state == PLAYER_STATE_ON_GROUND) {

        model->game_objects[player_num].object_velocity = PLAYER_INITIAL_JUMP_VELOCITY;
        model->game_objects[player_num].state = PLAYER_STATE_MOVING_UP;
