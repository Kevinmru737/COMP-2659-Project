...
void final_victory_animation(UINT32* back_buffer, UINT32* front_buffer,
UINT32* current_buffer);

void set_buffers(UINT32** back_buffer, UINT32** front_buffer,
UINT8 back_buffer_array[]);

void switch_buffers(UINT32** current_buffer, UINT32* front_buffer,
UINT32 * back_buffer);
void main_game_loop();
 ...
 *
 * Details:
 * - The function initializes the game state and manages the main game loop until
 *   a fatal collision occurs or the game reaches its final state.
 *
 * - Utilizes global variables to track the state of the game - 
 *   is_main_game_running, fatal_collision, game_state, render_request. 
 *   They must be reset at the start of each play through
 *
 * - Enable keyboard must be called for user input and the key board must be
 *   disabled after the user has either collided or reached max game state
 *
 * - Upon a fatal collision, plays an explosion sound and animation,
 *   then displays the score. If the game completes without a fatal
 *   collision, it triggers the final victory animation.
******************************************************************************/
void main_game_loop() {
 ...
 *
 * Details: The back buffer pointer is set to a 256-byte aligned address within
 *          the global back_buffer_array. Front buffer pointer is set to point
 *          to the value provided by the our custom get_video_base
 *
 * Input:  Back buffer, front buffer and the original true buffer
 * Output: Modifies the back buffer to point to a 256 byte aligned address 
 *         in back_buffer_array and changes the address stored at front buffer
 *******************************************************************************/
void set_buffers(UINT32** back_buffer, UINT32** front_buffer,
UINT8 back_buffer_array[]) {
 ...
 *        model - Pointer to the game model containing state and objects
 *                 to be rendered.
 *        is_curr_buffer_front - Pointer to a boolean flag indicating the 
 *        currently active buffer (TRUE for front, FALSE for back).
 *
 * Output: Animates a fatal collision by cycling through explosion images
 *         on the game display. 
******************************************************************************/
...
UINT32* current_buffer, Model *model, short *num_times_ground_drawn) {
    ...
    while(explosion_counter < EXPLOSION_ANIMATION_TOTAL_FRAMES) {
	    ...
	    if(render_request == 1) {
            render(model, current_buffer, num_times_ground_drawn);
            render_explosion(model, (UINT8 *)current_buffer, explosion_counter);
 ...
 *
 * Input: current_buffer - Double pointer to the current buffer's
 *                          base address.
 *        front_buffer - Pointer to the front buffer's base address.
 *        back_buffer - Pointer to the back buffer's base address.
 *
 * Output: Modifies the current_buffer pointer to switch between pointing
 *         to the front and the back buffer.
******************************************************************************/
void switch_buffers(UINT32** current_buffer, UINT32* front_buffer,
UINT32 * back_buffer) {
...
/******************************************************************************
 * Purpose: Similar to the main game loop, but collisions won't cause the game
            to end
 ******************************************************************************/
 ...
 *          Animates a final victory scene by cycling through victory images
 *          on the game display.
******************************************************************************/
void final_victory_animation(UINT32* back_buffer, UINT32* front_buffer,
UINT32* current_buffer){
    ...
    while (animation_loop_count < MAX_VICTORY_ANIMATION_LOOP_COUNT) {
        ...
        while(animation_frame_counter < FINAL_VICTORY_FRAME_COUNT) {
	        if(render_request == 1) {
                draw_8_bit_bitmap((UINT8 *)current_buffer, 0 ,0, 400, 640,
                victory_animation_frames[animation_frame_counter]);
                set_video_base(current_buffer);
                switch_buffers(&current_buffer, front_buffer, back_buffer);
