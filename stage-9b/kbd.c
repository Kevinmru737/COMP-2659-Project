...
void kbd_driver() {
    ...
    if (value >= 0xF8 || mouse_state > 0) {

        if(mouse_state == NO_MOUSE_STATE){
            mouse_state = MOUSE_2nd_PACKET_STATE;
        } else if( mouse_state == MOUSE_2nd_PACKET_STATE) {
            mouse_state = MOUSE_3rd_PACKET_STATE;
        } else{
            mouse_state = NO_MOUSE_STATE;
        }

    } else if((value & BREAK_CODE) == 0 || SHIFT_BREAK_CODES(value)) { /*only store makes and shift brake codes*/
...
/*
 * Purpose: Puts the scancode(value) in the input buffer. Will not put it in if the buffer is full
 */
 ...
 *          takes into account shift(left and right) and caps lock key.
 */
unsigned char get_value_from_buffer() {
    unsigned char value = kbd_buffer.buffer[kbd_buffer.front++];
    if(is_buffer_empty()) {
        return '\0';
    }
