 ...
 *
 * Details: Renders a 32x32 bitmap representing the player at a specified vertical position on the screen buffer.
 *          Adjusts base pointer according to y-coordinate for correct placement.
 *
 ...
 *
 * Details: Renders a bitmap representing the platform or a  triangle at a specified position on the screen buffer.
 *          Adjusts base pointer according to x and y coordinates 
 *
 * Input:   
 ...
 *          - is_short_platform: Boolean indicating if the platform is of a shorter type.
 *
 * Output:  Updates the screen buffer with the platform triangle's bitmap at the specified location.
 *********************************************************/
void draw_platform_triangle(UINT32 *base,int x,int y,const UINT32 *bitmap, bool is_short_platform )
{
	int row = 0;
	int height = is_short_platform ? PLATFORM_HEIGHT_SHORT : DEFAULT_OBJECT_HEIGHT;    /*ALl objects expect for a short platform have a height of 32*/
